  ...
  steps: [
    {
      ...
      settings: {
        ...
        content_type: "application/x-www-form-urlencoded",
        template: |||
          {{#success build.status}}icon=smile{{else}}icon=frown{{/success}}&message={{#each build.pipename}}1{{this}}:{{build.pipestatus[{{@index}}]}}{{/each}} Drone [{{repo.owner}}/{{repo.name}}](https://github.com/{{repo.owner}}/{{repo.name}}/commit/{{build.commit}}) ({{build.branch}}) **{{build.status}}** [({{build.number}})]({{build.link}}) by {{build.author}}
        |||
      },
    },
  ],
